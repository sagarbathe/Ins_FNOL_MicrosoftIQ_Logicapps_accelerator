"""
HTTP-triggered Azure Function wrapping tools_workiq_graph.search_documents()
...
from tools_workiq_graph import search_documents  # noqa: E402
...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    if not query:
        return func.HttpResponse(
            json.dumps({"error": "query is required"}), status_code=400, mimetype="application/json"
        )

    try:
        results = search_documents(query, top=top)
    except Exception as e:
        logging.exception("search_documents failed")
        return func.HttpResponse(
